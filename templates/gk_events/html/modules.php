...
function modChrome_gk_style($module, $params, $attribs) {	
	...
	if (!empty ($module->content)) {		
		...
		if($module->showtitle) {	
			...
			$title = preg_replace('/__(.*?)__/i', '<small>${1}</small>', $title);

			if($params->get('module_link_switch')) {
				echo '<h'.$header_type.' class="header'.($gkPage ? ' gkPage' : '').'"><a href="'. $params->get('module_link') .'">'. $title .'</a></h'.$header_type.'>';
			} else {
				echo '<h'.$header_type.' class="header'.($gkPage ? ' gkPage' : '').'">'. $title .'</h'.$header_type.'>';
			}
		}
